...
def create_json_data(voc_path_train,voc_path_test, output_path):
    '''
    ...
    with open(os.path.join(voc_path_train, "ImageSets/Main/trainval.txt")) as f:
        ids = f.read().splitlines()
    
    for id in ids:
        #Get object, image in XML file
        object_path = os.path.join(voc_path_train, "Annotations", id + ".xml")
        objects = parse_annotation(object_path)
        if len(objects) == 0:
            continue
        train_objects.append(objects)
        
        image_path = voc_path_train + "/JPEGImages/"+id+".jpg"
    ...
    with open(os.path.join(voc_path_test, "ImageSets/Main/test.txt")) as f:
        ids = f.read().splitlines()
        
    for id in ids:
        object_path = os.path.join(voc_path_test, "Annotations", id + ".xml")
        objects = parse_annotation(object_path)
        if len(objects) == 0:
            continue
        test_objects.append(objects)
        test_image_path = voc_path_test + "/JPEGImages/"+id+".jpg"
...
create_json_data("../VOC2007/VOCtrainval_06-Nov-2007/VOCdevkit/VOC2007",
                 "../VOC2007/VOCtest_06-Nov-2007/VOCdevkit/VOC2007",
                 "./JSONdata")
